...
def _get_all_predictions(pFilenames, modelParams):
    ...
    for i in range(0,len(pFilenames)):
        ...
        predAllListTemp[int(tMatJson['seq'])-INITINDEX].append(tMatJson)
    for i in range(9):       
...
def evaluate(modelParamsList, prevPParam=list()):
    ...
    for i in range(len(seqIDs)):
        ...
        for trnItr in range(len(modelParamsList)):
            ...
            ###### Get prediction map
            #### CREATE NTUPLE-2 TMATS FROM BEGINING OF GROUND TRUTH AND ATTACH TO THE BEGINING OF PREDICTIONS
            # create map
            pPoseParam = _get_prediction(predPoses, modelParams['seqIDs'][i])
            print("   Iteration: {0}".format(trnItr+1))
            print("         GT  pose  count  =", len(gtPose))
            print("         GT  param  count  =", len(gtParam))
            print("         Pred param count =", len(pPoseParam))
            print("         Num Tuples =", NUMTUPLES)
            pParam = list()
            for ntupID in range(NUMTUPLES-2):
                pParam.append(gtParam[ntupID])
            for poseID in range(len(pPoseParam)):
                pParam.append(pPoseParam[poseID]['tmat'])
            if "170808" in jsonsToRead[0]:
                pParam.append(gtParam[len(gtParam)-1])
        ...
        ## Reverse order
        gtMapOrig = gtMapOrig[::-1]
        for i in range(len(modelParamsList)):
            pMapSeqList[i]=pMapSeqList[i][::-1]
