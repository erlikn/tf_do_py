...
import Data_IO.tfrecord_io_2 as tfrecord_io
...
def _warp_w_orig_newOrig(imageOrig, imageDuo, pOrig, HAB, **kwargs):
    # update the Perturbed image given the original image

    # split for depth dimension
    orig, pert = np.asarray(np.split(imageDuo, 2, axis=2))
    pert = np.reshape(pert, [128,128])
    orig = orig.reshape([orig.shape[0], orig.shape[1]])

    # Get the correct box information
    # p & 0 is top left    - 1 is top ritfrecord_writerght
    # 2     is bottom left - 3 is bottom right
    pRow = int(pOrig[0][0])
    pCol = int(pOrig[1][0])
    squareSize = orig.shape[0]
    ...
    # get transformation matrix and transform the image to new space
    Hmatrix = cv2.getPerspectiveTransform(np.transpose(pOrig), np.transpose(pPert))
    orig = cv2.warpPerspective(imageOrig, Hmatrix, (imageOrig.shape[1], imageOrig.shape[0]))

    # crop the image at original location
    orig = orig[pRow:pRow+squareSize, pCol:pCol+squareSize]
...
def output(batchImageOrig, batchImage, batchPOrig, batchTHAB, batchPHAB, batchPHABPrevSum, batchTFrecFileIDs, **kwargs):
    """
    ...
    for i in range(kwargs.get('activeBatchSize')):
        ...
        cHAB = batchTHAB[i]-batchPHAB[i] # new target
        cHAB = np.asarray([[cHAB[0], cHAB[1], cHAB[2], cHAB[3]],
                           [cHAB[4], cHAB[5], cHAB[6], cHAB[7]]], np.float32)
        # PREDICTION SUM
        HABSum = batchPHABPrevSum[i]+batchPHAB[i]
        # put them in correct form
        HAB = np.asarray([[HABSum[0], HABSum[1], HABSum[2], HABSum[3]],
                          [HABSum[4], HABSum[5], HABSum[6], HABSum[7]]], np.float32)
        ...
        if kwargs.get('warpOriginalImage'):
            ...
            # NOT DEVELOPED YET
            patchOrig, patchPert = _warp_w_orig_newOrig(batchImageOrig[i], batchImage[i], pOrig, HAB, **kwargs)
        else:
        ...
        tfrecord_io.tfrecord_writer(batchImageOrig[i], patchOrig, patchPert, pOrig, cHAB, HAB,
