...
def normalize(image):
    ...
    coef = tf.constant(2.0, dtype=tf.float32)
...
def fetch_inputs(numPreprocessThreads=None, numReaders=1, **kwargs):
    ...
    with tf.name_scope('batch_processing'):
        ...
        for _ in range(numPreprocessThreads):
            ...
            images, pclA, pclB, target, tfrecFileIDs = tfrecord_io.parse_example_proto_DIFF(exampleSerialized, **kwargs)
            imageA, imageB = tf.split(images, [1,1], axis=2)
            imagediff = normalize(tf.subtract(imageB, imageA))
            images = tf.concat([imageA, imagediff, imageB], axis=2)
        ...
        print(batchImages.get_shape())
        imagesA, imagesDiff, imagesB = tf.split(batchImages, [1, 1, 1], axis=3)
        tf.summary.image('imagesA', imagesA)
        tf.summary.image('imagesD', imagesDiff)
        tf.summary.image('imagesB', imagesB)
