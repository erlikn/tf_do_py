...
def parse_example_proto(exampleSerialized, **kwargs):
    featureMap = {
        ...
        'pHABSum': tf.FixedLenFeature([8], dtype=tf.float32),
    ...
    HAB = features['HAB']
    pHABSum = features['pHABSum']
    pOrig = features['pOrig']

    #validate_for_nan()

    return imageOrig, image, pOrig, HAB, pHABSum, fileID

def tfrecord_writer(imageOrig, imgPatchOrig, imgPatchPert, pOrig, HAB, PHABsum, tfRecordFolder, tfFileName, fileID):
    """
    ...
    pHABSumRow = np.asarray([PHABsum[0][0], PHABsum[0][1], PHABsum[0][2], PHABsum[0][3],
                             PHABsum[1][0], PHABsum[1][1], PHABsum[1][2], PHABsum[1][3]], np.float32)
    pHABSumList = pHABSumRow.tolist()
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
        ...
        'pHABSum': _float_nparray(pHABSumList), # 2D np array
