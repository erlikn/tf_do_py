...
def fetch_inputs(numPreprocessThreads=None, numReaders=1, **kwargs):
    ...
    with tf.name_scope('batch_processing'):
        ...
        for _ in range(numPreprocessThreads):
            ...
            imageOrig, imageBuffer, pOrig, HAB, pHABSum, tfrecFileIDs = tfrecord_io.parse_example_proto(exampleSerialized, **kwargs)
            image = image_preprocessing(imageBuffer, **kwargs) # normalized between [-1,1]
            imagesHomographiesOrigsqrs.append([imageOrig, image, pOrig, HAB, pHABSum, tfrecFileIDs])

        batchImageOrig, batchImage, batchPOrig, batchHAB, batchpHABSum, batchTFrecFileIDs = tf.train.batch_join(imagesHomographiesOrigsqrs,
        ...
        return batchImageOrig, batchImage, batchPOrig, batchHAB, batchpHABSum, batchTFrecFileIDs
...
def inputs(**kwargs):
    ...
    with tf.device('/cpu:0'):
        batchImageOrig, batchImage, batchPOrig, batchHAB, batchPHABSum, tfrecFileID = fetch_inputs(**kwargs)
        
        if kwargs.get('usefp16'):
            batchImage = tf.cast(batchImage, tf.float16)
            batchHAB = tf.cast(batchHAB, tf.float16)
            batchPHABSum = tf.cast(batchPHABSum, tf.float16)

    return batchImageOrig, batchImage, batchPOrig, batchHAB, batchPHABSum, tfrecFileID
