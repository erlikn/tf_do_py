...
def inference(images, **kwargs): #batchSize=None, phase='train', outLayer=[13,13], existingParams=[]
    ...
    fireOut, prevExpandDim = model_base.conv_fire_parallel_inception_module('conv1', images, int(images.get_shape()[3]), numParalModules,
    ...
    fireOut, prevExpandDim = model_base.conv_fire_parallel_module('conv8', fireOut, prevExpandDim, numSeqModules,
                                                                  ...
                                                                  wd, stride=[1,2,2,1], **kwargs)
    ############# CONV9
    fireOut, prevExpandDim = model_base.conv_fire_parallel_inception_module('conv9', fireOut, prevExpandDim, numSeqModules,
                                                                  {'cnn1x1': modelShape[8]},#, 'cnn3x3': modelShape[6], 'cnn5x5': modelShape[6]},
                                                                  wd, **kwargs)
    ############# CONV10
    fireOut, prevExpandDim = model_base.conv_fire_parallel_module('conv10', fireOut, prevExpandDim, numSeqModules,
                                                                  {'cnn3x3': modelShape[9]},
                                                                  wd, stride=[1,1,2,1], **kwargs)
    ###### DROPOUT after CONV8
    ...
    fireOut1, prevExpandDim = model_base.conv_fire_inception_module('convFC', fireOut, prevExpandDim,
                                                       {'cnnFC': modelShape[8]},
                                                       wd, **kwargs)
    ...
    if kwargs.get('batchNorm'):
        fireOut = model_base.batch_norm('batch_norm', fireOut, dtype, kwargs.get('phase'))
